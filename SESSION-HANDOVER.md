...
### Dependabot PR sweep — final outcome
All 9 originally-open Dependabot PRs (#1–#9) resolved against the now-green `main`:
- **#1–#8** (`github_actions` version bumps: lighthouse-ci-action, checkout, action-release, github-script, action-setup, vercel-action, upload-artifact, setup-node, slack-github-action) — Quality Gates + E2E Tests passed on all eight after a rebase onto `main` post-#25; merged as-is (squash). Verified compatibility for the three deploy-only actions (`getsentry/action-release@v3`, `amondnet/vercel-action@v42`, `slackapi/slack-github-action@v4.0.0`) by reading each action's `action.yml` directly, since those steps live in `deploy-prod.yml`/`deploy-preview.yml`/`db-backup.yml`/`rollback.yml`/`sentry-alert.yml`, which don't run on PRs (`if: false` disables deploys, and preview/prod jobs need repo secrets) — confirmed the input/output names our workflows pass (`webhook`, `webhook-type`, `payload`, `environment`, `version`, `version_prefix`, `vercel-token`, `vercel-org-id`, `vercel-project-id`, `vercel-args`, `github-comment`, `preview-url`) are unchanged in the new major versions.
- **#9** (`production-dependencies`: sentry 9→10, jose 5→6, pino 9→10, resend 4→6, zod 3→4) — superseded by Dependabot's own rebase into #18, which this session validated locally and re-landed as PR #22 (one code fix: `ZodError.errors` → `.issues` in `src/app/actions/refunds.ts`, per zod 4's breaking change). #9 and #18 both auto-closed once #22 merged.

All eight `github_actions` PRs also surfaced a **new, unrelated finding**: Lighthouse CI now actually *runs* (previously always short-circuited by the Issue 1–7 breaks) and fails a real, pre-existing accessibility assertion — `/pricing` scores 0.92 against the configured `minScore: 0.95` (`.lighthouserc.json`). Confirmed pre-existing and unrelated to any of this session's changes: identical failure reproduces across all 8 action-bump PRs regardless of which action they touch, and `mergeable_state: "unstable"` (not `"blocked"`) confirms Lighthouse isn't a required check, so it didn't block merging. Not investigated or fixed here — out of scope for a dependency-drift triage and needs an actual accessibility audit of `/pricing`. Left as-is for a follow-up pass.

Two other open PRs, **#17** (Jules/Sentinel: async password hashing DoS fix) and **#23** (Jules/Bolt: grading-engine Map lookup perf), are unrelated automated PRs outside this task's scope and were left untouched.
