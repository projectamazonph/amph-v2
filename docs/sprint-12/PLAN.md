...
**Acceptance**
- Runbook exists at `docs/runbooks/production-deploy.md`.
- Lists all 11 env vars by name (8 from Sprint 11 + 3 existing).
- Smoke-test script lives at `scripts/smoke-prod.sh` and exits 0 against a
  fresh deployment.
- Rollback procedure references the exact Vercel CLI command and the
...
**Acceptance**
- Neon scheduled backup enabled (or external cron invokes `pg_dump`).
- `docs/runbooks/db-backup-restore.md` documents the procedure.
...
- Security headers verified via `curl -I https://<preview>/...`.
- Multi-tenant query audit doc at `docs/security/tenant-isolation.md` lists
  each endpoint and its guard.

### STORY-056 — Production deploy (1 pt)
...
**Acceptance**
- Production URL returns 200 for `/`, `/dashboard`, `/pricing`, `/courses`.
...
### STORY-057 — Launch communications (1 pt)
...
**Acceptance**
- Social copy approved and scheduled.
- Resend broadcast scheduled (or sent immediately after deploy).
- Launch retro doc `docs/sprint-12/RETRO.md` written within 48h of launch.
...
- [ ] All 5 stories shipped.
- [ ] Production URL passes smoke script.
- [ ] Sentry + Slack verified live.
- [ ] RETRO written.
- [ ] 52 / 52 stories complete.
