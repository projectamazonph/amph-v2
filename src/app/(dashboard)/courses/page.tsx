...
export default async function CoursesIndexPage() {
  ...
  // Get user's lesson progress (optimized: scope query to published lessons only to avoid loading full history)
  const allPublishedLessonIds = courses.flatMap((c) =>
    c.modules.flatMap((m) => m.lessons.map((l) => l.id))
  );

  const lessonProgress = allPublishedLessonIds.length > 0
    ? await db.lessonProgress.findMany({
        where: { userId: user.id, lessonId: { in: allPublishedLessonIds }, deletedAt: null },
