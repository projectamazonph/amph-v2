...
export const signUpAction = createSafeAction(signUpSchema, async (data) => {
  ...
  if (!ipRl.allowed) {
    throw new Error(`Too many signup attempts from this IP. Try again in ${ipRl.retryAfterSeconds}s.`);
  }
...
export const signInAction = createSafeAction(signInSchema, async (data) => {
  ...
  if (!ipRl.allowed) {
    throw new Error(`Too many login attempts from this IP. Try again in ${ipRl.retryAfterSeconds}s.`);
  }
