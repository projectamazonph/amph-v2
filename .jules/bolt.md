...
**Learning:** Querying a user's entire history of lesson progress on dashboard and course catalog pages creates large database payloads and high memory consumption, which grows quadratically as more courses/lessons are added.
**Action:** Always scope database queries for lesson progress to the relevant courses or specific lesson IDs (e.g., using `lessonId: { in: lessonIds }`) to load only what is being displayed.
